...
public class ComprasDAO {
    ...
    public boolean insertar(ComprasDTO dto) {
        ...
        try (Connection con = Conexion.getConexion();
            ...
            ps.executeUpdate();
            return true;
        } catch (SQLException e) { System.err.println("Error insertar compra: " + e.getMessage()); return false; }
    }

    // Todas las compras (para el admin)
    public List<ComprasDTO> listarTodas() { return consultar(-1); }

    // Solo las compras de un cliente (para el usuario normal)
    public List<ComprasDTO> listarPorCliente(int idCliente) { return consultar(idCliente); }
    ...
    private List<ComprasDTO> consultar(int idCliente) {
        ...
        } catch (SQLException e) { System.err.println("Error listar compras: " + e.getMessage()); }
        return lista;
    }
}